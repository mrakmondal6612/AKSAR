...
export const handleGetAdminStats = async (req: AuthenticatedAdminRequest, res: Response) => {
  try {
    ...
    const completedRedemptions = await Redemption.find({ status: RedemptionStatus.COMPLETED }).populate("reward");
    ...
    const itemMap: { [key: string]: { name: string; type: string; count: number; pointsSpent: number } } = {};
    ...
    const farmingAlerts: any[] = [];
    for (const [userId, points] of Object.entries(userPointsMap)) {
      if (points >= 300) {
        const u = await User.findOne({ uniqueId: userId });
        if (u) {
          farmingAlerts.push({
            _id: u._id.toString(),
            username: u.userName,
    ...
    const totalEarnedPoints = allTransactions
      .filter((t) => t.type === TransactionType.EARNED || (t.type === TransactionType.ADMIN_ADJUSTMENT && t.points > 0))
      .reduce((sum, t) => sum + t.points, 0);
    
    const totalSpentPoints = Math.abs(
      allTransactions
        .filter((t) => t.type === TransactionType.SPENT || (t.type === TransactionType.ADMIN_ADJUSTMENT && t.points < 0))
        .reduce((sum, t) => sum + t.points, 0)
    );

...
import { awardPoints } from "../../services/reward.service";
 ...
 */
export async function handleGetStoreRewards(req: Request, res: Response) {
  try {
 ...
 */
export async function handleClaimDailyLogin(req: AuthenticatedRequest, res: Response) {
 ...
 */
export async function handleRedeemReward(req: AuthenticatedRequest, res: Response) {
  const userUniqueId = req.userUniqueId;
  ...
  try {
    ...
    const user = await User.findOne({ uniqueId: userUniqueId }).session(session);
    if (!user) {
      throw new Error("User not found");
    }

    const reward = await Reward.findOne({ rewardId, isActive: true }).session(session);
    if (!reward) {
 ...
 */
export async function handleGetMyRewards(req: AuthenticatedRequest, res: Response) {
  const userUniqueId = req.userUniqueId;
  if (!userUniqueId) {
    return res.status(401).json({ success: false, message: "Unauthorized" });
  }
  ...
  try {
    const redemptions = await Redemption.find({ user: userUniqueId })
      .populate("reward")
      .sort({ createdAt: -1 });
 ...
 */
export async function handleGetTransactionHistory(req: AuthenticatedRequest, res: Response) {
  const userUniqueId = req.userUniqueId;
  if (!userUniqueId) {
    return res.status(401).json({ success: false, message: "Unauthorized" });
  }
  ...
  try {
    const history = await Transaction.find({ user: userUniqueId }).sort({ createdAt: -1 });
 ...
 */
export async function handleAdminAdjustPoints(req: Request, res: Response) {
  ...
  try {
    const user = await User.findOne({ uniqueId: studentUniqueId });
    if (!user) {
 ...
 */
export async function handleAdminGetAnalytics(req: Request, res: Response) {
  try {
    // 1. Point aggregates
    const aggregateEarned = await Transaction.aggregate([
      { $match: { points: { $gt: 0 } } },
      { $group: { _id: null, total: { $sum: "$points" } } },
    ]);
    const aggregateSpent = await Transaction.aggregate([
      { $match: { points: { $lt: 0 } } },
      { $group: { _id: null, total: { $sum: "$points" } } },
    ]);
    ...
    const redemptionCounts = await Redemption.aggregate([
      {
        $group: {
          _id: "$reward",
          count: { $sum: 1 },
    ...
    const farmingAlerts = await Transaction.aggregate([
 ...
 */
export async function handleAdminAddReward(req: Request, res: Response) {
 ...
 */
export async function handleAdminUpdateReward(req: Request, res: Response) {
  const { rewardId } = req.params;
  const updates = req.body;

  try {
    const reward = await Reward.findOneAndUpdate({ rewardId }, updates, { new: true });
    if (!reward) {
 ...
 */
export async function handleAdminDeleteReward(req: Request, res: Response) {
  const { rewardId } = req.params;

...
const handleApprovePost = async (req, res) => {
    try {
        ...
        await post.save();
        if (oldStatus !== CommunityPost_model_2.PostStatus.APPROVED && (post.category?.toLowerCase() === "notes" || post.category?.toLowerCase() === "note")) {
            try {
                const uploader = await User_model_1.default.findById(post.user);
                if (uploader) {
                    const { awardPoints } = await Promise.resolve().then(() => __importStar(require("../../services/reward.service")));
                    await awardPoints(uploader.uniqueId, 30, "NOTE_UPLOAD", `Uploaded useful notes: approved by admin`, `notes_approved_${uploader.uniqueId}_${postId}`);
                }
            }
            catch (rewardError) {
